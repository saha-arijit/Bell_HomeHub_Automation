...
Generate_TestCases_All
    ...
    Set Test Documentation    To generate the test case combination for All    #Library Command    Tests Name (mention the test case Name)    Test Type (select from TP, RR, Lat or MaxClient)    Load Mode (Auto/Manual)    Direction (Bi-Direction = 1 / Uni-Direction = 0)
    ...    # Band Selection (2.4 / 5)    Channel    # Frame Size    Load Values (Calculated by ROBOTAutomation during execution if Load Mode set to Atuo)    Expected Connections    Source
    ...    # Destination    Test Duration    # MCS    SS    Channel Bandwidth    Guard Interval
    ...    # Ethernet Port    Wireless Port    # Group Type    Save PCAPS    Throughput Multiplier
